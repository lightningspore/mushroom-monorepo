...
def is_alpha(c):
    return (
        c >= "a" and c <= "z" or c >= "A" and c <= "Z"  # lowercase alpha
    )  # uppercase alpha
...
def validate_name_char(c, num):
    return is_alpha(c) or (num and is_numeric(c)) or c == "_" or c == ":"
...
class Counter(Metric):
    ...
    def render(self, namespace):
        ...
        for l, v in self.values.items():
            lines.append(
                "{}{} {}".format(
                    render_name(namespace, self.name),
                    render_labels(self.labelKeys, l),
                    v,
                )
            )
